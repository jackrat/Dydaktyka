...
namespace Microsoft.Data.ConnectionUI
{
  ...
  {
    public DataConnectionDialog(InitMode initMode)
    {
      InitializeComponent();
      ...
      switch (initMode)
      {
        case InitMode.OnlyMsSql: InitForMsSql(); break;
        case InitMode.OnlyOracle: InitForOracle(); break;
        default: InitForAll(); break;
      }
    }
    ...
    private void InitForOracle()
    {
      ...
      initDSrcDict();
      initProvDict();
      SelectedDataSource = DataSource.OracleDataSource;
    }



    private Dictionary<string, DataSource> dataSources;
    private Dictionary<string, DataProvider> dataProviders;

    private void initDSrcDict()
    {
      dataSources = new Dictionary<string, DataSource>();
      foreach (DataSource ds in DataSources)
        if (ds.Name != null)
          dataSources.Add(ds.Name, ds);
      dataSources.Add(UnspecifiedDataSource.DisplayName, UnspecifiedDataSource);
    }

    private void initProvDict()
    {
      dataProviders = new Dictionary<string, DataProvider>();
      foreach (DataProvider dp in UnspecifiedDataSource.Providers)
        dataProviders.Add(dp.Name, dp);
    }
    ...
    private void InitForAll()
    {
      ...
      DataSources.Add(SqlCe.SqlCeDataSource);
    ...
    private void MyParseConnectionStr(string aConnStr)
    {
      string dataSrc = null;
      string provider = null;
      string[] pairs = aConnStr.Split(';');
      foreach (string pair in pairs)
      {
        string[] KeyValue = pair.Split('=');
        if (KeyValue[0].Trim().ToLower() == "data source") dataSrc = KeyValue[1].Trim();
        if (KeyValue[0].Trim().ToLower() == "provider") provider = KeyValue[1].Trim();
      }

      //DataSource ds = null;
      //if (!String.IsNullOrEmpty(dataSrc) && this.dataSources.TryGetValue(dataSrc, out ds))
      //{
      //  SelectedDataSource = ds;
      //}

      //DataProvider dp = null;
      //if (!String.IsNullOrEmpty(provider) && this.dataProviders.TryGetValue(provider, out dp))
      //{
      ...
      if (SelectedDataSource != null)
        ...
        {
          DataProvider dp = null;
          foreach (DataProvider idp in SelectedDataSource.Providers)
            dp = idp;
          SelectedDataProvider = dp;
        }
        else
      ...
      try { InnerConnectionString = aConnStr; }
      catch { }
    }
    ...
    public string ConnectionString
    {
      get { return InnerConnectionString; }
      set { MyParseConnectionStr(value); }
    }
  ...
  public enum InitMode { All, OnlyMsSql, OnlyOracle }
